...
let identity_sso_cookie_exchange_promise;

P.register('identity_sso_api', function() {
    ...
    function initAuth(authPortalConfig, option, ssoTargetApp, callbackFunction) {
        ...
        if (ssoAuthCode === null) {
            ...
            if (exchangeAuthCodeForCookies()) {
                const result = {result: 'sso_success'};
                callbackFunction(result);
            } else {
                const result = {result: 'sso_failure'};
                callbackFunction(result);
            }
        }
    }
    ...
    return {initAuth: initAuth,
        exchangeAuthCodeForCookies: exchangeAuthCodeForCookies}
});

// P.when('A', 'ready', 'identity_sso_api').execute("identity_sso_init", function (A, ready, identitySsoApi) {

//     // detech authCode, and do AJAX cookie exchange

//     P.declare('identity_sso_cookie_exchange_promise', new Promise(function(resolve, reject) {

//         if (identity_sso_api.exchangeAuthCodeForCookies())
//         {
//             resolve();
//         }
//       })
